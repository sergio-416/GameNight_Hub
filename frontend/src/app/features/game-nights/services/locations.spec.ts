...
describe('LocationsService', () => {
	...
	describe('fetch locations', () => {
		it('should return list of game night locations from API', () => {
			...
			service.getAllLocations().subscribe((locations: Location[]) => {
	...
	describe('update location', () => {
		it('should update location fields when valid data provided', () => {
			...
			service.updateLocation(locationId, updateData).subscribe((location: Location) => {
	...
	describe('delete location', () => {
		it('should remove location when valid id provided', () => {
			...
			service.deleteLocation(locationId).subscribe((location: Location) => {
				expect(location._id).toBe(locationId);
			});
	...
	describe('find in bounds', () => {
		it('should return only locations within specified bounding box', () => {
			...
			const locationsInBounds: Location[] = [
				{ _id: '1', name: 'Cafe Inside', latitude: 41.38, longitude: 2.15, venueType: 'cafe' },
				{ _id: '2', name: 'Store Inside', latitude: 41.4, longitude: 2.18, venueType: 'store' },
			];

			service.findInBounds(swLat, swLng, neLat, neLng).subscribe((locations: Location[]) => {
		...
		it('should return empty array when no locations in specified bounds', () => {
			...
			const req = httpMock.expectOne(`${apiUrl}/locations/bounds?swLat=0&swLng=0&neLat=0&neLng=0`);
			req.flush([]);
		});

		it('should include venue types in query when provided', () => {
			const swLat = 41.3;
			const swLng = 2.1;
			const neLat = 41.5;
			const neLng = 2.2;
			...
			service
				.findInBounds(swLat, swLng, neLat, neLng, venueTypes)
				.subscribe((locations: Location[]) => {
					expect(locations).toEqual(locationsInBounds);
				});

			const expectedUrl = `${apiUrl}/locations/bounds?swLat=${swLat}&swLng=${swLng}&neLat=${neLat}&neLng=${neLng}&venueType=cafe,store`;
			const req = httpMock.expectOne(expectedUrl);
			expect(req.request.method).toBe('GET');
			req.flush(locationsInBounds);
		});
